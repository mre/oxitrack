...
async function count() {
  ...
  try {
    ...
    // keepalive: true ensures the request outlives the page unload —
    // async/await cannot be used here since beforeunload does not wait for promises.
    window.addEventListener("beforeunload", () => {
      timeOnPageMs += new Date().getTime() - startTime.getTime();
      const timeOnPageS = Math.round(0.001 * timeOnPageMs);
      fetch("{{ base_url }}/page-left/" + visitorId + "/" + timeOnPageS, {
        ...fetchOptions,
        keepalive: true,
      });
    });

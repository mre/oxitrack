...
Add the following script tag to your website, replacing `OXITRACK_BASE_URL` with the base URL of your OxiTrack instance:
...
On the first request to a new path, OxiTrack sends a request to that path prefixed by `tracked_origin_callback`. If the response status is 2xx, the path is accepted. Otherwise it is rejected.

### Binary

```sh
cargo install oxitrack --locked
```

Set the `OXITRACK_CONFIG_FILE` environment variable to point to your config file (defaults to `config.toml` in the current directory).

## Configuration

The config file path is read from `OXITRACK_CONFIG_FILE` (defaults to `config.toml`). Every option can also be set or overridden via environment variable.

| Parameter                 | Description                                                                                                        | Default          | Environment variable               |
| ------------------------- | ------------------------------------------------------------------------------------------------------------------ | ---------------- | ---------------------------------- |
| `socket_address`          | Listening address. Use `127.0.0.1:8080` for local testing; `0.0.0.0:80` for containers.                          | `"0.0.0.0:80"`   | `OXITRACK_SOCKET_ADDRESS`          |
| `base_url`                | Base URL of your OxiTrack instance. Used to build `count.js`.                                                     |                  | `OXITRACK_BASE_URL`                |
| `tracked_origin`          | [Origin](https://developer.mozilla.org/en-US/docs/Glossary/Origin) of your tracked website (used for CORS).      |                  | `OXITRACK_TRACKED_ORIGIN`          |
| `tracked_origin_callback` | Origin used for path validation requests. Useful when OxiTrack and your site are on the same local network.       | `tracked_origin` | `OXITRACK_TRACKED_ORIGIN_CALLBACK` |
| `min_delay_secs`          | Seconds a visitor must spend on the page before the visit is counted.                                             | `19`             | `OXITRACK_MIN_DELAY_SECS`          |
| `utc_offset.hours`        | Hours component of your UTC offset                                                                                 | `0`              | `OXITRACK_UTC_OFFSET__HOURS`       |
| `utc_offset.minutes`      | Minutes component of your UTC offset                                                                               | `0`              | `OXITRACK_UTC_OFFSET__MINUTES`     |
...
base_url = "https://oxitrack.your_domain.com"
...
OxiTrack logs to stdout at the `info` level by default. Set `RUST_LOG` to one of `off`, `error`, `warn`, `info`, `debug`, or `trace` to change the level.

...
pub async fn get(
    ...
    let DaysSinceFirstVisit {
        ...
        ..
    } = DaysSinceFirstVisit::build(&state.db, path_id).await?;

    if days_since_first_visit <= 60 {
        DataPoint::all::<ContiguousDay>(&state.db, path_id, None).await
    } else if days_since_first_visit <= 1826 {
        // Less than 5 years (about 60 months).
        DataPoint::all::<ContiguousMonth>(&state.db, path_id, None).await
    } else {
        DataPoint::all::<ContiguousYear>(&state.db, path_id, None).await
    }
}

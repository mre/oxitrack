-- From https://www.2ndquadrant.com/en/blog/postgresql-10-identity-columns/
-- The only modification is the last line to set "always".
...
BEGIN
  ...
  -- Replace "by default" with "always"
  EXECUTE 'ALTER TABLE ' || tbl || ' ALTER id SET GENERATED ALWAYS';
END;
$$;

SELECT upgrade_serial_to_identity('paths', 'id');
SELECT upgrade_serial_to_identity('visits', 'id');
